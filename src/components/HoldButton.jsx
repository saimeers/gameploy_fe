...
/**
 * Botón que exige mantener pulsado para confirmar una acción irreversible.
 * Portado de React Bits (variante TypeScript) a JSX.
 */
...
export default function HoldButton({
  ...
  const drive = (to, duration, ease) => {
    ...
    m.from = m.p
    m.to = to
    // drive() solo corre desde los manejadores de puntero y teclado, nunca en el render.
  ...
  const complete = () => {
    ...
    // complete() se dispara desde requestAnimationFrame o desde un temporizador.
    // eslint-disable-next-line react-hooks/purity
  ...
  // Referencia siempre al último `release`, para que los listeners de window
  // no capturen una versión obsoleta.
  ...
  const handlePointerDown = (e) => {
    ...
    } catch { /* el navegador puede rechazar la captura */ }
  }
  ...
  return (
    ...
    >
      ...
      <span id={hintId} className="absolute h-px w-px overflow-hidden whitespace-nowrap [clip-path:inset(50%)]">
        Mantén pulsado {Math.round(holdTime / 100) / 10} segundos para confirmar
      </span>
    </button>
  )
}
